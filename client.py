...
def listen_for_offer():
    ...
    except socket.error as err:  # Added exception handling here
        print(f"Socket error in listen_for_offer: {err}")
    finally:
        udp_socket.close()  # Ensure the socket is closed on error or exit
...
def send_udp_request(server_ip, server_udp_port):
    try:  # Added exception handling here
        ...
        print(f"Error in send_udp_request: {err}")
    finally:
...
def start_client():
    ...
    try:
        ...
        print(f"Unexpected error in start_client: {err}")
